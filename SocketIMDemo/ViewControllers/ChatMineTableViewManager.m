...
@interface ChatMineTableViewManager () {
    ...
    /**需要展示的日志数据*/
    NSMutableArray<IMServerLog*> *_logs;
}
@end
...
- (instancetype)initWithLogs:(NSMutableArray<IMServerLog*>*)logs tableView:(UITableView*)tableView {
    if(self = [super init]) {
...
- (void)updateLogs:(NSMutableArray<IMServerLog*>*)logs {
    _logs = [logs mutableCopy];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    IMServerLog *clientLog = _logs[indexPath.row];
